...
export function calculateInvestmentAllocation(
  ...
  const cashAsset = assets.find((asset) => asset.name === "cash")
    ?.startingValue;
  const cryptoAsset = assets.find((asset) => asset.name === "crypto")
    ?.startingValue;
  const preciousMetalsAsset = assets.find(
    (asset) => asset.name === "preciousMetals",
  )?.startingValue;
  const realEstateAsset = assets.find((asset) => asset.name === "realestate")
    ?.startingValue;
  const stocksAsset = assets.find((asset) => asset.name === "stocks")
    ?.startingValue;
  const otherAsset = assets.find((asset) => asset.name === "other")
    ?.startingValue;
