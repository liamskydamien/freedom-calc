...
export const useGenerateWealthAndCost = (
  ...
  const freeCashflow: number[] = useStretchArray(
    freeCashflowPerPhase,
    timeBetweenEachPhase,
  );

  // Calculate Wealth
  const wealth: number[] = useCalculateWealth(
    assetGroups,
    freeCashflow,
    realTimeframe,
    investmentWeights,
  ...
  const cost: number[] = useCalculateCosts(
    annualCosts,
    inflationRate,
    realTimeframe,
  );
