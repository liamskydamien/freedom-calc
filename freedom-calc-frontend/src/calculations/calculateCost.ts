...
export function calculateCost(
    ...
    const costWithInflation : number[] = annualCosts.map((cost) => { // Calculate the cost with inflation
        return cost * (1 + inflationRate);
    });

    let totalCosts : number = costWithInflation.reduce((sum, cost) => sum + cost, 0) / reductionFactor; // Calculate the total cost
