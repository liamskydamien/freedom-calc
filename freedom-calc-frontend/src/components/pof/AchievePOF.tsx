...
type AchievePOFProps = {
    t: any;
    pof: Point | null;
}
const AchievePOF :  React.FC<AchievePOFProps> = ({t, pof}) => {
    return (
        <div className="alert alert-info">
            ...
            <div className="flex flex-col">
                <span className="text-left">{t('pof_title')}</span>
                {
                    pof ?
                        <span className="text-content2">{t('pof_achieved')} {pof.x} {t('pof_achieved_age')}</span>
                        :
                        <span className="text-content2">{t('pof_not_achieved')}</span>
                }
            </div>
        </div>
    )
