...
const InputAssetAllocation : React.FC<InputAssetAllocationProps> = ({t, crypto, setCrypto, other, setOther, preciousMetals, setPreciousMetals, realEstate , setRealEstate, setCash, cash, stocks, setStocks}) => {
    ...
    const handleRealEstateChange = (event: { target: { value: string}; }) => {
        setRealEstate(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }
    ...
    const handlePreciousMetalsChange = (event: { target: { value: string}; }) => {
        setPreciousMetals(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }
