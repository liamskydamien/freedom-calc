...
const LiabilitiesInput : React.FC<LiabilitiesInputProps> = ({liabilitiesValid, liabilitiesState, liabilities,t}) => {

    const {objectRelatedLiabilities, setNonObjectRelatedLiabilities, nonObjectRelatedLiabilities, setOtherLiabilities, setObjectRelatedLiabilities, setProvisions, provisions,otherLiabilities} = liabilitiesState;

    /**
     * Updates the liabilities array with the new values
     */
    const updateLiabilities = () => {
        liabilities.startingValue = objectRelatedLiabilities + nonObjectRelatedLiabilities + otherLiabilities + provisions;
    }
    ...
    */
    useEffect(() => {
        updateLiabilities();
    }, [otherLiabilities, objectRelatedLiabilities, nonObjectRelatedLiabilities, provisions]);

    const objectRelatedLiabilitiesChangeHandler = (event: { target: { value: any; }; }) => {
    ...
    const nonObjectRelatedLiabilitiesChangeHandler = (event: { target: { value: any; }; }) => {
        const value = event.target.value;
        // This will allow only numbers and empty string to be set
        if (value === '' || /^[0-9\b]+$/.test(value)) {
