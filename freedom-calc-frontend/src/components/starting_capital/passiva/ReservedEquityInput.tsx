...
import {LiabilitiesState, ReservedEquityState} from "../../../models/types/AssetContextTypes";
...
const ReservedEquityInput : React.FC<ReservedEquityInputProps> = ({liabilitiesValid, reservedEquityState, reservedEquity ,t}) => {

    const {reservedEquityForConsumption, setReservedEquityForOther, reservedEquityForOther, setReservedEquityForConsumption, setReservedEquityForPensionProvision, reservedEquityForPensionProvision} = reservedEquityState;

    /**
     * Updates the liabilities array with the new values
     */
    const updateReservedEquity = () => {
        reservedEquity.startingValue = reservedEquityForConsumption + reservedEquityForOther + reservedEquityForPensionProvision;
    }
    ...
    */
    ...
    }, [reservedEquityForConsumption, reservedEquityForOther, reservedEquityForPensionProvision]);

    const reservedEquityForConsumptionChangeHandler = (event: { target: { value: any; }; }) => {
    ...
    const reservedEquityForOtherChangeHandler = (event: { target: { value: any; }; }) => {
        const value = event.target.value;
        // This will allow only numbers and empty string to be set
        if (value === '' || /^[0-9\b]+$/.test(value)) {
    ...
    const reservedEquityForPensionProvisionChangeHandler = (event: { target: { value: any; }; }) => {
        const value = event.target.value;
        // This will allow only numbers and empty string to be set
        if (value === '' || /^[0-9\b]+$/.test(value)) {
            setReservedEquityForPensionProvision(value === '' ? 0 : parseFloat(value));
        }
    }
