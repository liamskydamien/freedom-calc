...
const AddIncome : React.FC<AddIncomeProps> = ({ t, setIncome, setStatus, income}) => {
    ...
    const dependentChangeHandler = (event: { target: { value: string; }; }) => {
        setDependent(event.target.value === "" ? 0 :parseFloat(event.target.value));
    }

    const selfemployedChangeHandler = (event: { target: { value: string; }; }) => {
        setSelfemployed(event.target.value === "" ? 0 :parseFloat(event.target.value));
    }

    const rentingIncomeChangeHandler = (event: { target: { value: string; }; }) => {
        setRentingIncome(event.target.value === "" ? 0 :parseFloat(event.target.value));
    }

    const capitalIncomeChangeHandler = (event: { target: { value: string; }; }) => {
        setCapitalIncome(event.target.value === "" ? 0 :parseFloat(event.target.value));
    }
    ...
    const otherIncomeChangeHandler = (event: { target: { value: string; }; }) => {
        setOtherIncome(event.target.value === "" ? 0 :parseFloat(event.target.value));
    }
