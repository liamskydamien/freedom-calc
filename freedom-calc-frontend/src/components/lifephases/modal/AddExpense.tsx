...
const AddExpense : React.FC<AddExpenseProps> = ({ t, setExpense, setStatus, expense}) => {
    ...
    const addExpenseHandler = () => {
        const expense : Expenses = new Expenses(taxes, rent ,consumption, socialSecurity ,maintenance, insurance, interest, otherCosts);
        setExpense(expense);
        setStatus("summary");
    }
    ...
    const consumptionChangeHandler = (event: { target: { value: string; }; }) => {
        setConsumption(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }

    const insuranceChangeHandler = (event: { target: { value: string; }; }) => {
        setInsurance(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }

    const otherCostsChangeHandler = (event: { target: { value: string; }; }) => {
        setOtherCosts(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }
    ...
    const socialSecurityChangeHandler = (event: { target: { value: string; }; }) => {
        setSocialSecurity(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }

    const maintenanceChangeHandler = (event: { target: { value: string; }; }) => {
        setMaintenance(event.target.value === "" ? 0 : parseFloat(event.target.value));
    }
