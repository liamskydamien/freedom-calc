...
export const useCalculateLifeLine = ( assetGroups: AssetGroup[],
                                      lifephases : LifePhase[],
                                      timeframe : number,
                                      inflationRate : number,
                                      investmentWeights : InvestmentWeights,
                                      startAge : number) => {
    const lifeline = useGenerateWealthAndCost(assetGroups, lifephases, timeframe, startAge, investmentWeights, inflationRate);
    const {pof , cost, wealth, freeCashflow} = lifeline;
    const {ciGraph, wealthGraph} : CIGraph = useCalculateCIGraph(freeCashflow, wealth);
