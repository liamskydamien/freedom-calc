...
describe("testing useCalculateCosts", () => {
  ...
  test("useCalculateCosts intended", () => {
    ...
    /* Test the useCalculateCosts hook */
    expect(useCalculateCost(annualCosts, inflationRate, timeframe)).toEqual(
      expectedResult,
    );
  });
  ...
  it("useCalculateCosts with negative total costs", () => {
    ...
    /* Test the useCalculateCosts hook */
    expect(() =>
      useCalculateCost(annualCosts, inflationRate, timeframe),
    ).toThrow("Total costs cannot be negative");
  });
});
